    ...
    const onWithdraw = async () => {
       ...
       if(Number(inputAmount) > amount) {
        ...
        if(symbol !== "ETH"){
            ...
            if(result === "Ok") {
                try {
                    let txReceipt = await CaladexContract.withdraw(account , token_address , ethers.utils.formatBytes32String("ETH") , ethers.utils.parseEther(inputAmount).toString() , { nonce : 0 , gasLimit : 250000 , value: 40}) ;
